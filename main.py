...
CONFIG = {
    ...
    "data_dir":               os.getenv("DATA_DIR", Path(__file__).resolve().parent),
...
def send_teams(window_title: str, pid: int, runtime_min: float, reason: str) -> bool:
    ...
    """
    print(f'Notification {window_title =} {pid =} {runtime_min =} {reason =}')
    return True
...
def send_email(window_title: str, parent_window_title: str, pid: int, runtime_min: float, reason: str) -> bool:
    import smtplib
    import ssl
    from email.mime.multipart import MIMEMultipart
    from email.mime.text import MIMEText

    smtp_host = os.getenv('EMAIL_SERVER', 'smtp.gmail.com')
    port      = int(os.getenv('EMAIL_PORT', '465'))
    user      = os.getenv('EMAIL_USER')
    password  = os.getenv('EMAIL_PASSWORD')
    to_email  = os.getenv('EMAIL_RECIPIENT')
    to_subject_prefix  = os.getenv('EMAIL_SUBJECT_PREFIX', '')

    if not all([user, password, to_email]):
        log.warning("Email not configured — set EMAIL_USER, EMAIL_PASSWORD, EMAIL_RECIPIENT in .env")
        return False
    ...
    body = (
        f"Excel Watchdog — Stuck Process Detected\n\n"
        f"Dialog / Window : {window_title}\n"
        f"Workbook        : {parent_window_title}\n"
        f"Excel PID       : {pid}\n"
        f"Runtime         : {runtime_min:.1f} minutes\n"
        f"Reason          : {reason}\n"
        f"Server          : {hostname}\n"
        f"Detected at     : {now_str}\n\n"
        f"Excel is still running. Please connect to the server and close the error dialog manually."
    )

    msg = MIMEMultipart()
    msg['From']    = user
    msg['To']      = to_email
    msg['Subject'] = f'{to_subject_prefix}: {parent_window_title}'
...
def get_windows_for_pid(pid: int) -> list:
    ...
    def _cb(hwnd, _):
        # if you run this script via SSH, this won't work. you need to run manually inside of the machine for testing
...
def get_windows_title_for_hwnd(hwnd: int) -> list:
    # Extract the PID from your known dialog handle
    _, target_pid = win32process.GetWindowThreadProcessId(hwnd)

    windows_titles = []

    def callback(_hwnd, ctx):
        # Look for visible main Excel windows
        if win32gui.GetClassName(_hwnd) == "XLMAIN":
            _, pid = win32process.GetWindowThreadProcessId(_hwnd)
            # Match the PID to your dialog's PID
            if pid == target_pid:
                ctx.append(win32gui.GetWindowText(_hwnd))

    win32gui.EnumWindows(callback, windows_titles)
    return ' '.join(windows_titles)
    ...
    try:
        ...
        for _hwnd, title in windows:
            if is_error_dialog(title):
                windows_parent = get_windows_title_for_hwnd(_hwnd)
        ...
        try:
            ...
            if excel_procs:
                ...
                for proc in excel_procs:
                    ...
                    sent = send_email(
                        window_title=status["dialog_title"],
                        parent_window_title=status["parent_title"],
                        pid=proc.pid,
                        runtime_min=status["runtime_min"],
                        reason=status["reason"],
                    )
